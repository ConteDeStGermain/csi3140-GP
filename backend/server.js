...
app.post('/saveMessage', (req, res) => {
    ...
    let attitude = getSentiment(message).trim().split(" ");
    let senti = attitude[0];
    let sentiScore = attitude[1];

    // If this ID already exists, add the new message to its array. Otherwise, create a new array for this ID.
    if (messages[id]) {
        messages[id].push({message, senti, sentiScore});
    } else {
        messages[id] = [{message, senti, sentiScore}];
    }

    // Write the new messages object back to the file
    fs.writeFileSync(filePath, JSON.stringify(messages));

    res.status(200).json({ status: 'success', attitude: senti, attitudeScore: sentiScore });
});
